...
class HackInstrument implements Instrument {
  ...
  float[] detune = {-0.15f, 0.0f, 0.15f}; // デチューンのそれぞれのズレ
  ...
  HackInstrument( float frequency, float maxAmp, Waveform wf ) { 
    ...
    for (int i = 0; i < 3; i++) {
      ...
      lfoVibrato[i] = new Oscil(60.0f+0.5f*i, 2.0f, Waves.SINE);
      // LFO(ビブラート用)のエンベロープの設定 (最大振幅, A, D, S, R)
      vibratoAdsr[i] = new ADSR(1.0,  0.2, 1.0, 0.65, 0.15);
...
void setup() 
{
  ...
  currentWaveform = WavetableGenerator.gen10(
      4096, // サンプルサイズ(2の倍数で)
      // 各倍音の振幅値
      new float[] { 0.224f, 0.447f, 0.708f, 1.000f, 0.447f,
                    0.891f, 0.562f, 0.316f, 0.282f, 0.282f,
                    0.158f, 0.112f, 0.071f, 0.045f, 0.020f,
                    0.025f, 0.011f, 0.010f, 0.006f, 0.007f }  //トランペット
      );
